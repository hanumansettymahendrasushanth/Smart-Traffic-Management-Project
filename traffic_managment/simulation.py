...
nextGreen =  (currentGreen+1)%noOfSignals   # Indicates which signal will be green next
...
speeds = {'car':2.25, 'bus':2.8, 'truck':2.8, 'rickshaw':2, 'bike':2.5}  # average speeds of vehicles
        ...
        for j in range(len(vehicles[directionNumbers[nextGreen]][i])):
            ...
            if(vehicle.crossed==0):
                ...
                if(vclass=='car'):
                    ...
                    noOfRickshaws += 1
    # print(noOfCars)
    greenTime = ((noOfCars*carTime) + (noOfRickshaws*rickshawTime) + (2*noOfBuses*busTime) + (2*noOfTrucks*truckTime)+ (noOfBikes*bikeTime))
...
laneVehicleCount = {'right': 0, 'down': 0, 'left': 0, 'up': 0}  # Initialize dictionary to count vehicles in each lane

def generateVehicles():
    while True:
        # Generate vehicles as before
        ...
        # Increment the count of vehicles in the corresponding lane
        lane = directionNumbers[direction_number]
        laneVehicleCount[lane] += 1
